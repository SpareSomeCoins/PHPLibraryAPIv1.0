...
class SpareSomeAPI 
{
    ...
    public function send($to, $amount, $curName = NULL, $referral = NULL) {
        ...
        if (array_key_exists("status", $response) && $response["status"] == 200) {
            return array(
                ...
                'html' => htmlspecialchars(rtrim(rtrim(sprintf("%.8f", $amount/100000000), '0'), '.')) . ' ' . $curName . ' was sent to <a target="_blank" href="https://sparesomecoins.com/check/'. rawurlencode($to) . '">your <b>SpareSomeCoins.com</b> account.</a>.',
                'html_coin' => htmlspecialchars(rtrim(rtrim(sprintf("%.8f", $amount/100000000), '0'), '.')) . ' '.$this->currency.' was sent to <a target="_blank" href="https://sparesomecoins.com/check/' . rawurlencode($to) . '">your <b>SpareSomeCoins.com</b> account.</a>.',
